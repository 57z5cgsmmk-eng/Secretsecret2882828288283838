-- ============================================================
-- IBLIS HUB - Partie 1/5
-- ============================================================
if _G.YoutRunning then
    warn("[IBLIS HUB] Instancia previa detectada. Se recomienda rejoin si hay comportamiento errático.")
end
...
if not LP then
    warn("[IBLIS HUB] LocalPlayer no disponible en 15s. Abortando.")
    return
end

local pgui = LP:WaitForChild("PlayerGui", 20)
if not pgui then
    warn("[IBLIS HUB] PlayerGui no cargó en 20s. Abortando.")
    return
end
...
CONFIG_FILE = "Iblis_" .. tostring(LP.UserId) .. ".json"
BAT_V2_HIT_DIST = 4.5
...
function stopAutoSteal()
    ...
    if progressFill then
        TS:Create(progressFill, TweenInstance.new(0.2), { Size = UDim2.new(0, 0, 1, 0) }):Play()
    end
    if progressPct then progressPct.Text = "0%" end
end
...
local function createESP()
    ...
    esp.Highlight.Name = "IblisESP_Highlight"
    ...
    nameBillboard.Name = "IblisESP_Name"
...
function setupSpeedIndicator(char)
    ...
    discordLabel.Text = "discord.gg/SrBSCVyKH"
...
-- [IBLIS] Marcador azul de ultima posicion
do
    ...
    local function createOrUpdateMarker(targetCFrame)
        ...
        if not marker or not marker.Parent then
            ...
            marker.Name         = "IblisTPBatLastPosition"
...
local function _alIsProtectedVisual(object)
    ...
    while parent and parent ~= workspace do
        ...
        if name:match("^Crystal") or name:match("^Eclipse") or name:match("^ESP_")
            or name:match("^BloodHounds") or name:match("^Yout") or name:match("^Iblis") then
            return true
        end
...
function saveAllSettings(force)
    ...
    if not readFn or not writeFn then
        warn("[IBLIS Save] Executor file API is unavailable.")
        return false
    end
    ...
    if ok then _lastSavedJSON = json else warn("[IBLIS Save] Write or verification failed.") end
    return ok
end
...
do
    ...
    if readFn and writeFn then
        local oldPath = "Yout_" .. tostring(LP.UserId) .. ".json"
        ...
        if okOld and type(old) == "string" and not okNew then
            ...
            warn("[IBLIS] Config migrada de " .. oldPath .. " a " .. CONFIG_FILE)
        end
    end
end
...
function buildGui()
    ...
    local GUI_NAMES = {"Yout", "IblisHub", "YoutMobilePanel", "TpBatButton", "InstaResetButton",
    ...
    scriptLogo.Image = "rbxassetid://133146589354241"
...
-- =====================================================================
-- TP BAT FLOATING BUTTON
-- =====================================================================
function createTpBatFloatingButton()
    local panel = Instance.new("ScreenGui")
    panel.Name = "IblisTpBatButton"
    panel.ResetOnSpawn = false
    panel.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
...
-- =====================================================================
-- INSTA RESET FLOATING BUTTON
-- =====================================================================
function createInstaResetFloatingButton()
    local panel = Instance.new("ScreenGui")
    panel.Name = "IblisInstaResetButton"
    panel.ResetOnSpawn = false
    panel.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
...
local function _bootSection(name, fn)
    print("[IBLIS HUB Boot] Iniciando:", name)
    local ok, err = pcall(fn)
    if not ok then
        print("[IBLIS HUB Boot] FALLÓ:", name, tostring(err))
        table.insert(_bootErrors, {name = name, err = tostring(err)})
        warn("[IBLIS HUB Boot] Error en '" .. name .. "': " .. tostring(err))
    else
        print("[IBLIS HUB Boot] OK:", name)
    end
    return ok
end
...
if gui and main then
    ...
    if #_bootErrors > 0 then
        task.defer(function()
            pcall(function()
                ...
                if type(writeFn) == "function" then
                    local log = "[IBLIS HUB Boot Errors] " .. os.date("%Y-%m-%d %H:%M:%S") .. "\n"
                    for _, e in ipairs(_bootErrors) do
                        log = log .. e.name .. ": " .. e.err .. "\n"
                    end
                    pcall(writeFn, "Iblis_boot_errors.txt", log)
                    warn("[IBLIS HUB] Errores escritos a Iblis_boot_errors.txt")
                end
            end)
        end)
        ...
        task.defer(function()
            pcall(function()
                ...
                title.Text = "⚠ IBLIS HUB: " .. #_bootErrors .. " sección(es) fallaron"
    ...
    warn("[IBLIS HUB Boot] Fatal: buildGui falló. El script no puede continuar.")
end
